...
class CaptchaModel(nn.Module):
    ...
    def forward(self, images, targets=None):
        bs, _, _, _ = images.size() # batch_size, h, w, channels
        x = F.relu(self.conv_1(images))
        x = self.pool_1(x)
        x = F.relu(self.conv_2(x))
        x = self.pool_2(x) # (Batch_size = ..., kernel_size = 64, height = 18, width = 75)

        # perform RNN on width axis
        x = x.permute(0, 3, 1, 2) # => (Batch_size,width,kernel_size,height)
        ...
        x = self.drop_1(x)
        x, _ = self.lstm(x)
        x = self.output(x) # (batch size, time steps, values)
        x = x.permute(1, 0, 2)

        if targets is not None:
            # To use CTC loss we have to define input & label size for each batch
            # and log values ..
